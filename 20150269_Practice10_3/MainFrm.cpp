	...
	: m_nViewFlag(0)
{
	...
	m_nViewFlag = FIRST_VIEW;						// 처음에 FIRST_VIEW (폼 뷰)로 설정.
...
void CMainFrame::OnViewChange()
{
	...
	if (m_nViewFlag == SECOND_VIEW)				// 뷰가 일반 뷰인 상태에서 [보기/뷰 전환]
	{											// 메뉴를 선택하면 FirstView() 호출
		FirstView();
		m_nViewFlag = FIRST_VIEW;
	}
	else if (m_nViewFlag == FIRST_VIEW)			// 뷰가 폼 뷰인 상태에서 [보기/뷰 전환]
...
{
	SetActiveView(m_pFirstView);						// 폼 뷰를 활성화 시킨다.
	m_pSecondView->ShowWindow(SW_HIDE);					// 에디트 뷰를 숨긴다.
	m_pFirstView->ShowWindow(SW_SHOW);					// 폼 뷰를 화면에 보이게 한다.
	m_pFirstView->SetDlgCtrlID(AFX_IDW_PANE_FIRST);		// 메시지가 발생할 경우
...
{
	m_pFirstView = (CMy20150269_Practice10_3View*)GetActiveView();			// 현재 활성화 되어있는
																			// 뷰를 얻는다.
	if (m_pSecondView == NULL)						// 객체가 아직 생성된 적이 없다면 if 블록실행
	{
		m_pSecondView = new CSecondView;		// 객체 생성
		CCreateContext context;					// 도큐먼트에 대한 정보가 있는 객체 생성
	...
	SetActiveView(m_pSecondView);				// 두 번째 뷰를 활성화 시킨다.
	m_pSecondView->ShowWindow(SW_SHOW);			// 활성화 시킨 두 번째 뷰를 화면에 표시
